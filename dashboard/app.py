...
Self-contained: imports no pipeline code. It reads ONLY the saved outputs
produced by steps 06-08 (metrics tables, figures, the frozen prediction file
and explainability tables), so it starts instantly and always shows exactly
the numbers reported in the paper.

To run the Streamlit dashboard (from the repository root):
...
# ---------------------------------------------------------------------------
# Paths — resolved relative to this file, keeping the dashboard self-contained
# ---------------------------------------------------------------------------
ROOT = Path(__file__).resolve().parent.parent
TABLES_DIR = ROOT / "outputs" / "tables"
FIGURES_DIR = ROOT / "outputs" / "figures"
DATA_PROCESSED_DIR = ROOT / "data" / "processed"
FINAL_CONFIG_PATH = ROOT / "models" / "final_model_config.json"
...
def predictions() -> pd.DataFrame | None:
    p = DATA_PROCESSED_DIR / "test_predictions.parquet"
    return pd.read_parquet(p) if p.exists() else None


@st.cache_data
def config() -> dict | None:
    return (json.loads(FINAL_CONFIG_PATH.read_text())
            if FINAL_CONFIG_PATH.exists() else None)
...
with t_perf:
    ...
    p = FIGURES_DIR / "fig07_test_confusion_matrix.png"
    if p.exists():
        st.markdown('<div class="card">', unsafe_allow_html=True)
